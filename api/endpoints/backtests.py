...
from litadel.agents.utils.strategy_code_generator_agent import create_strategy_code_generator, validate_strategy_code
...
# Request model for strategy code generation
class GenerateStrategyCodeRequest(BaseModel):
    """Request to generate Python strategy code."""

    strategy_description: str = Field(..., description="Natural language strategy description")
    ticker: str = Field(..., description="Ticker symbol for the strategy")
    indicators: list[str] = Field(default_factory=list, description="List of indicators to use (RSI, SMA, MACD, etc.)")
    entry_conditions: dict = Field(default_factory=dict, description="Entry conditions")
    exit_conditions: dict = Field(default_factory=dict, description="Exit conditions")
    risk_params: dict = Field(default_factory=dict, description="Risk management parameters")
...
class ExtractParametersRequest(BaseModel):
    ...
    current_form_state: dict = Field(
        default_factory=dict, description="Current state of the form to avoid re-extracting"
    )
...
class ExecuteTradingIntentRequest(BaseModel):
    ...
    strategy_code_python: str | None = Field(None, description="Pre-generated Python code (optional)")
...
async def create_backtest(
    ...
    strategy_code = request.strategy_code_python
    ...
    return BacktestResponse(
        ...
        strategy_code_python=backtest.strategy_code_python,
...
async def update_backtest(
    ...
    if request.strategy_code_python is not None:
        backtest.strategy_code_python = request.strategy_code_python
    if request.strategy_type is not None:
        backtest.strategy_type = request.strategy_type
...
async def get_backtest_strategy(
    ...
    """Get strategy Python code for review."""
    backtest = _verify_backtest_ownership(backtest_id, user.id, db)
...
async def generate_strategy_code(
    request: GenerateStrategyCodeRequest = Body(...),
    user: User = Depends(get_current_user_jwt),
):
    """
    Generate Python strategy code from requirements using LLM.

    This endpoint streams the generated code in real-time as the LLM produces it,
    ...
    async def generate_stream():
        ...
        try:
            logger.info(f"User {user.username} requested strategy code generation (streaming)")
            ...
            # Create the strategy code generator agent
            ...
            logger.info(f"Successfully streamed strategy code for user {user.username}")

        except Exception as e:
            logger.exception(f"Failed to generate strategy code: {e}")
            # Send error event
            error_data = json.dumps({"type": "error", "message": f"Failed to generate strategy code: {e!s}"})
...
# Execution endpoints removed - backtest execution engine has been removed
...
async def extract_backtest_parameters(
    ...
    try:
        ...
        # Structure response
        # Parse clarification questions, handling missing 'field' attribute
        clarification_questions = []
        for q in result.get("clarification_questions", []):
            try:
                # Ensure 'field' exists, default to 'unknown' if missing
                if "field" not in q:
                    q["field"] = q.get("question", "unknown_field")[:50]  # Use question as field name
                clarification_questions.append(ClarificationQuestion(**q))
            except Exception as e:
                logger.warning(f"Failed to parse clarification question: {q}, error: {e}")
                continue
...
@router.post("/execute-intent", status_code=status.HTTP_201_CREATED)
async def execute_trading_intent(
    ...
    """
    Execute a trading intent (create backtest or analysis record).

    Based on the detected intent and parameters, this endpoint will:
    - Backtest: Create a backtest record (in pending state)
    - Live Trading: (Future) Set up live trading strategy
    - Analysis: Create an analysis request

    Note: Backtest execution must be triggered separately (execution engine removed).
    """
    ...
    if intent == "backtest":
        ...
        # Use provided code or create placeholder
        strategy_code = request.strategy_code_python
        if not strategy_code:
            # Generate simple buy-and-hold strategy as placeholder
            ticker_name = ticker_list[0] if ticker_list else "TICKER"
            strategy_code = f"""# Auto-generated strategy for: {name}
...
"""
        ...
        # Create backtest (in pending state)
        backtest = Backtest(
            user_id=user.id,
            name=name,
            ...
            strategy_type="single_ticker" if len(ticker_list) == 1 else "portfolio",
        ...
        # Note: Execution must be triggered separately (execution engine removed)
        return {
            "success": True,
            "backtest_id": backtest.id,
            "message": f"Backtest '{name}' created in pending state. Execution engine not available.",
        }
