...
export const BacktestDetail: React.FC = () => {
  ...
  // Fetch trades from backtest (stored in same record now)
  ...
  // Fetch equity curve from backtest
        ...
        <div className="flex-1">
          <div className="flex items-center gap-4 mb-2">
            <h1 className="text-2xl font-bold text-primary font-mono">
              {displayBacktest.name}
            </h1>
            <StatusBadge status={displayBacktest.status as 'pending' | 'running' | 'completed' | 'failed'} />
            {displayBacktest.status === 'running' && (
              <span className="text-muted-foreground font-mono text-sm">
                Progress: {displayBacktest.progress_percentage}%
              </span>
            )}
          </div>
          {displayBacktest.description && (
            <p className="text-muted-foreground font-mono text-sm">
              {displayBacktest.description}
            </p>
          )}
          <p className="text-muted-foreground font-mono text-sm mt-2">
            {formatDateShort(displayBacktest.start_date)} - {formatDateShort(displayBacktest.end_date)} | Initial Capital: {formatCurrency(displayBacktest.initial_capital)}
          </p>
        </div>
      </div>

      {/* Performance Metrics */}
      {displayBacktest.status === 'completed' && (
        <Card className="p-6">
          ...
          <div className="grid grid-cols-5 gap-6">
            <MetricCard
              label="TOTAL RETURN"
              value={formatPercentageWithSign(displayBacktest.total_return_pct)}
            />
            <MetricCard
              label="SHARPE RATIO"
              value={displayBacktest.sharpe_ratio !== null ? displayBacktest.sharpe_ratio.toFixed(2) : 'N/A'}
            />
            <MetricCard
              label="MAX DRAWDOWN"
              value={formatPercentageWithSign(displayBacktest.max_drawdown_pct)}
            />
            <MetricCard
              label="WIN RATE"
              value={formatPercentageWithSign(displayBacktest.win_rate)}
            />
            <MetricCard
              label="TOTAL TRADES"
              value={displayBacktest.total_trades !== null ? displayBacktest.total_trades : 'N/A'}
            />
          </div>
        </Card>
      )}
      ...
      <Collapsible title="STRATEGY DEFINITION" defaultExpanded={false}>
        <div className="mb-6">
          ...
          <div className="text-foreground text-sm font-mono leading-relaxed">
            {displayBacktest.strategy_description}
          </div>
        </div>
        <div>
          <div className="text-muted-foreground text-xs mb-2 font-bold">
          ...
          <StrategyCodeVisualizer
            codeContent={displayBacktest.strategy_code_python}
            strategyType={displayBacktest.strategy_type}
          />
        </div>
      </Collapsible>

      {/* Pending State - Show Execute Button */}
      {displayBacktest.status === 'pending' && (
        <Card>
        ...
        <Card>
          <CardContent className="p-8 text-center">
            <p className="text-primary font-mono text-base mb-4">
              Backtest is running... Progress: {displayBacktest.progress_percentage}%
            </p>
            <div className="w-full h-1 bg-primary/20 mb-4">
              <div
                className="h-full bg-primary transition-all duration-300 ease-out"
                style={{ width: `${displayBacktest.progress_percentage}%` }}
              />
            </div>
            <Button
