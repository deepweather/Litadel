    ...
    try:
            ...
            try:
                ...
                if s_dt <= d <= e_dt:
                    ...
                    open_price = find_value(
                        [f"1a. open ({market_upper})", f"1b. open ({market_upper})", "1a. open", "1. open"]
                    )
                    high_price = find_value(
                        [f"2a. high ({market_upper})", f"2b. high ({market_upper})", "2a. high", "2. high"]
                    )
                    low_price = find_value(
                        [f"3a. low ({market_upper})", f"3b. low ({market_upper})", "3a. low", "3. low"]
                    )
        ...
        if csv_text.strip() == "time,open,high,low,close,volume" or len(rows) == 0:
            ...
            for date_str in all_dates:
                ...
                open_price = find_value(
                    values, [f"1a. open ({market_upper})", f"1b. open ({market_upper})", "1a. open", "1. open"]
                )
                high_price = find_value(
                    values, [f"2a. high ({market_upper})", f"2b. high ({market_upper})", "2a. high", "2. high"]
                )
                low_price = find_value(
                    values, [f"3a. low ({market_upper})", f"3b. low ({market_upper})", "3a. low", "3. low"]
                )
                close_price = find_value(
                    values, [f"4a. close ({market_upper})", f"4b. close ({market_upper})", "4a. close", "4. close"]
                )

                out.write(
                    f"{date_str},{open_price},{high_price},{low_price},{close_price},{values.get('5. volume', '')}\n"
                )
            return out.getvalue()
